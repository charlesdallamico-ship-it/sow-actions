...
export function AcceptResetPage({ token, onSuccess }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/accept-reset`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${ANON_KEY}` },
        body: JSON.stringify({ token, password }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? 'Erro ao redefinir senha.'); setBusy(false); return; }
      setDone(true);
      ...
      <div className="w-full max-w-md">
        ...
        {done ? (
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-8 text-center">
            ...
            <h2 className="text-lg font-bold text-slate-900 mb-2">Senha redefinida!</h2>
            <p className="text-sm text-slate-500">Sua senha foi atualizada. Redirecionando para o login...</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Redefinir senha</h2>
                <p className="text-xs text-slate-500">Defina uma nova senha para sua conta</p>
              </div>
            </div>
