...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(JSON.stringify({ error: "Sessão do convite não encontrada" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!callerData.user) {
      return new Response(JSON.stringify({ error: "Sessão do convite inválida ou expirada" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (invite.user_id !== callerData.user.id) {
      return new Response(JSON.stringify({ error: "Este convite não pertence a esta conta" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (invite.used_at) {
    ...
    // Activate profile
    const { data: acceptedUser } = await supabaseAdmin.auth.admin.getUserById(invite.user_id);
    await supabaseAdmin.from("profiles").update({ active: true, email_confirmed_at: acceptedUser.user?.email_confirmed_at ?? new Date().toISOString(), company_assignment_status: "approved" }).eq("user_id", invite.user_id);
